...
function startServer(connection, runtime) {
  ...
  var formatterMaxNumberOfEdits = Number.MAX_VALUE; // After the server has started the client sends an initialize request. The server receives
  // in the passed params the rootPath of the workspace plus the client capabilities.

  connection.onInitialize(function (params) {
    var _params$initializatio, _params$initializatio2, _params$initializatio3, _params$initializatio4, _params$initializatio5, _params$initializatio6, _params$initializatio7;

    var handledProtocols = (_params$initializatio = params.initializationOptions) === null || _params$initializatio === void 0 ? void 0 : _params$initializatio.handledSchemaProtocols;
    ...
    dynamicFormatterRegistration = getClientCapability('textDocument.rangeFormatting.dynamicRegistration', false) && typeof ((_params$initializatio2 = params.initializationOptions) === null || _params$initializatio2 === void 0 ? void 0 : _params$initializatio2.provideFormatter) !== 'boolean';
    ...
    formatterMaxNumberOfEdits = ((_params$initializatio3 = params.initializationOptions) === null || _params$initializatio3 === void 0 ? void 0 : (_params$initializatio4 = _params$initializatio3.customCapabilities) === null || _params$initializatio4 === void 0 ? void 0 : (_params$initializatio5 = _params$initializatio4.rangeFormatting) === null || _params$initializatio5 === void 0 ? void 0 : _params$initializatio5.editLimit) || Number.MAX_VALUE;
    var capabilities = {
      ...
      documentRangeFormattingProvider: ((_params$initializatio6 = params.initializationOptions) === null || _params$initializatio6 === void 0 ? void 0 : _params$initializatio6.provideFormatter) === true,
      documentFormattingProvider: ((_params$initializatio7 = params.initializationOptions) === null || _params$initializatio7 === void 0 ? void 0 : _params$initializatio7.provideFormatter) === true,
      colorProvider: {},
  ...
  var limitExceededWarnings = function () {
    var pendingWarnings = {};

    var showLimitedNotification = function showLimitedNotification(uri, resultLimit) {
      var warning = pendingWarnings[uri];
      connection.sendNotification(ResultLimitReachedNotification.type, "".concat(vscode_uri_1.Utils.basename(vscode_uri_1.URI.parse(uri)), ": For performance reasons, ").concat(Object.keys(warning.features).join(' and '), " have been limited to ").concat(resultLimit, " items."));
      warning.timeout = undefined;
    };

    return {
      cancel: function cancel(uri) {
        var warning = pendingWarnings[uri];
      ...
      onResultLimitExceeded: function onResultLimitExceeded(uri, resultLimit, name) {
        return function () {
          var warning = pendingWarnings[uri];
          ...
          if (warning) {
            ...
            warning.timeout.dispose();
            warning.timeout = runtime.timer.setTimeout(function () {
              return showLimitedNotification(uri, resultLimit);
            }, 2000);
          } else {
            warning = {
              features: _defineProperty({}, name, name)
            };
            warning.timeout = runtime.timer.setTimeout(function () {
              return showLimitedNotification(uri, resultLimit);
            }, 2000);
  ...
  var validateEnabled = true; // The settings have changed. Is send on server activation as well.

  connection.onDidChangeConfiguration(function (change) {
    var _settings$json, _settings$json2, _settings$json2$valid, _settings$json3, _settings$json4;

    var settings = change.settings;

    if (runtime.configureHttpRequests) {
      var _settings$http, _settings$http2;

      runtime.configureHttpRequests(settings === null || settings === void 0 ? void 0 : (_settings$http = settings.http) === null || _settings$http === void 0 ? void 0 : _settings$http.proxy, !!((_settings$http2 = settings.http) !== null && _settings$http2 !== void 0 && _settings$http2.proxyStrictSSL));
    }
    ...
    updateConfiguration();
    foldingRangeLimit = Math.trunc(Math.max(((_settings$json3 = settings.json) === null || _settings$json3 === void 0 ? void 0 : _settings$json3.resultLimit) || foldingRangeLimitDefault, 0));
    resultLimit = Math.trunc(Math.max(((_settings$json4 = settings.json) === null || _settings$json4 === void 0 ? void 0 : _settings$json4.resultLimit) || Number.MAX_VALUE, 0)); // dynamically enable & disable the formatter

    if (dynamicFormatterRegistration) {
      var _settings$json5, _settings$json5$forma;

      var enableFormatter = (_settings$json5 = settings.json) === null || _settings$json5 === void 0 ? void 0 : (_settings$json5$forma = _settings$json5.format) === null || _settings$json5$forma === void 0 ? void 0 : _settings$json5$forma.enable;
  ...
  connection.onNotification(SchemaContentChangeNotification.type, function (uriOrUris) {
    ...
    if (needsRevalidation) {
      var _iterator3 = _createForOfIteratorHelper(documents.all()),
          _step3;
  ...
  connection.onRequest(ForceValidateRequest.type, function (uri) {
    return new Promise(function (resolve) {
      var document = documents.get(uri);

      if (document) {
        updateConfiguration();
        validateTextDocument(document, function (diagnostics) {
          resolve(diagnostics);
        });
      } else {
        resolve([]);
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              return _context.abrupt("return", languageService.getLanguageStatus(document, jsonDocument));
  ...
  function updateConfiguration() {
    ...
    languageService.configure(languageSettings); // Revalidate any open text documents
  ...
  function validateTextDocument(textDocument, callback) {
    var respond = function respond(diagnostics) {
      connection.sendDiagnostics({
        uri: textDocument.uri,
    ...
    languageService.doValidation(textDocument, jsonDocument, documentSettings).then(function (diagnostics) {
      ...
      connection.console.error((0, runner_1.formatError)("Error while validating ".concat(textDocument.uri), error));
    });
  }
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              jsonDocument = getJSONDocument(document);
              return _context2.abrupt("return", languageService.doComplete(document, textDocumentPosition.position, jsonDocument));
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              ...
              jsonDocument = getJSONDocument(document);
              return _context3.abrupt("return", languageService.doHover(document, textDocumentPositionParams.position, jsonDocument));
  ...
  connection.onDocumentSymbol(function (documentSymbolParams, token) {
    return (0, runner_1.runSafe)(runtime, function () {
      ...
      if (document) {
        ...
        var onResultLimitExceeded = limitExceededWarnings.onResultLimitExceeded(document.uri, resultLimit, 'document symbols');

        if (hierarchicalDocumentSymbolSupport) {
          return languageService.findDocumentSymbols2(document, jsonDocument, {
            resultLimit: resultLimit,
            onResultLimitExceeded: onResultLimitExceeded
          });
        } else {
  ...
  connection.onDocumentColor(function (params, token) {
    return (0, runner_1.runSafeAsync)(runtime, /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee4() {
      var document, onResultLimitExceeded, jsonDocument;
        ...
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              ...
              onResultLimitExceeded = limitExceededWarnings.onResultLimitExceeded(document.uri, resultLimit, 'document colors');
              jsonDocument = getJSONDocument(document);
              return _context4.abrupt("return", languageService.findDocumentColors(document, jsonDocument, {
                resultLimit: resultLimit,
                onResultLimitExceeded: onResultLimitExceeded
              }));
  ...
  connection.onFoldingRanges(function (params, token) {
    return (0, runner_1.runSafe)(runtime, function () {
      ...
      if (document) {
        var onRangeLimitExceeded = limitExceededWarnings.onResultLimitExceeded(document.uri, foldingRangeLimit, 'folding ranges');
        return languageService.getFoldingRanges(document, {
          rangeLimit: foldingRangeLimit,
          onRangeLimitExceeded: onRangeLimitExceeded
        });
      }
        ...
        while (1) {
          switch (_context5.prev = _context5.next) {
            case 0:
              ...
              jsonDocument = getJSONDocument(document);
              return _context5.abrupt("return", languageService.findLinks(document, jsonDocument));
